...
!! preprocessing library
  ...
  implicit none
    ...
    double precision, intent(in) :: deltat,t0,f0
    integer, intent(in) :: NSTEP, Wscale
    integer, intent(in) :: is_laplace, is_window, window_type,mute_near,mute_far
    double precision, intent(in) :: dis_sr, S_x, S_t,V_slow,V_fast,offset_near,offset_far
    integer, intent(out) :: istart,iend

   integer :: itime,NA
   double precision :: tas(NSTEP)
...
if(is_window .eq. 1.and. istart<iend) then
  ...
  if(istart<iend) then 
      ! window
      tas(1:NSTEP)=0.d0
      call taper(NSTEP,istart,iend,window_type,tas)
      d(1:NSTEP)=d(1:NSTEP)*tas(1:NSTEP)
   endif 
endif ! window
...
!----------------------------------------------------------------------
  ...
  subroutine taper(npts,istart,iend,window_type,tas)
  use user_parameters
  implicit none

    integer, intent(in) :: npts, istart,iend, window_type
    double precision, dimension(*), intent(out) :: tas

    integer ::  i, nlen
    double precision :: sfac1, ipwr_t
    double precision, dimension(npts) :: fac


    nlen = iend - istart+1

    ! some constants
    sfac1 = (2./dble(nlen))**2   ! for Welch taper
    ipwr_t = 10                  ! for time-domain cosine taper
    ...
    do i = 1, nlen
      ...
      fac(i) = 1 - sfac1*((i-1) - dble(nlen)/2.)**2
 ...
 !! return 
   tas(istart:iend)=fac(1:nlen)

 end subroutine taper

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine window(x,NSTEP,B1,B2,B3,B4)
 use user_parameters
  implicit none

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! input parameters
  integer, intent(in) :: B1,B2,B3,B4, NSTEP
 ...
 ! set the taper properties according to type
      omega_left = PI/dble(n_width_left)
      omega_right = PI/dble(n_width_right)
      f0 = 0.5
      f1 = 0.5
  ! apply the taper symmetrically to left side of the data
  do i = 1, n_width_left
    x(B1-1+i) = x(B1-1+i) * (f0-f1*cos(omega_left*(i-1)))
  enddo
  do i = 1, n_width_right
   !  print*,i,omega_right,n_width_right,(f0-f1*cos(omega_right*(i-1)))
    x(B4+1-i) = x(B4+1-i) * (f0-f1*cos(omega_right*(i-1)))
  end do
...
! outputs:
  ...
  double precision, dimension(*), intent(out) :: cc_array
...
! outputs:
  double precision, dimension(*), intent(out) :: c_array

  ! local variables
  integer :: i_left, i_right, i,j,k
  double precision,dimension(npts1+npts2-1) :: cc_array
  double precision :: cc


  if (npts3.lt.0 .or. npts3.gt.npts1+npts2-1) then
    write(*,*) 'output length is improper: ', npts3
    return
  endif

  i_left =2
  i_right =npts1+npts2-1
  ! i is the index to shift to be applied to DATA (d)
k=0
  do i = i_left, i_right
k=k+1
...
! inputs
  ...
  double precision, intent(in) :: sig,c
...
! inputs
  double precision, dimension(*), intent(in) :: x
  double precision, intent(in) :: fc,sig,c
  integer, intent(in) :: npts

! outputs:
  double precision, dimension(*), intent(out) :: fe, f
...
subroutine WT(seism,NSTEP,level,NA)
  ...
  double precision, dimension(*), intent(out) :: seism
  integer, intent(in) :: NSTEP,level
  integer :: iend,istart,i,j,st
  integer, intent(out) :: NA
  double precision,dimension(:,:),allocatable :: basis, seism_WT
  double precision :: wc 
  double precision,dimension(:), allocatable :: nonzero_basis
