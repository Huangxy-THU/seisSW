...
!! copyright
  ...
  implicit none
    ...
    double precision, dimension(*), intent(in) :: dat1,dat2
    double precision, intent(in) ::  deltat,tshift_cc,dlnA_cc 
    integer, intent(in) :: i_fstart,i_fend
    double precision, dimension(*), intent(in) :: wvec
    complex*16, dimension(NPT), intent(out) :: trans_func
    double precision, dimension(*), intent(out) :: dtau_w,dlnA_w,err_dtau,err_dlnA

    ! FFT parameters
    double precision :: ampmax,wtr_mtm_use,wtr_use
    integer :: i,ictaper,iom
    integer :: i_ampmax

   !! transfer function
    complex*16, dimension(NPT) :: dat2_dtwo, dat1_dtwo, dat2_dtw_ho, dat1_dtw_ho,  &
                                  top_mtm,bot_mtm
   !! mutltitapered 
    double precision, dimension(NPT) :: dat1_dtw_h,dat2_dtw_h
    double precision, dimension(NPT) :: phi_mtm,dtau_mtm,abs_mtm,dlnA_mtm
    double precision, dimension(NPT) :: ey1,ey2
    double precision :: edt_ave,edt_iom,edA_ave,edA_iom
    double precision, dimension(:,:),allocatable :: tas,phi_mul,dtau_mul,abs_mul,dlnA_mul
   ...
   if( DISPLAY_DETAILS) then
      open(2,file=trim(output_dir)//'/tapers',status='unknown')
      do i = 1,nlen
         write(2,'(f15.2,<ntaper>e15.5)') (i-1)*deltat,tas(i,1:ntaper)
      enddo
      close(2)
    endif
...
!! estimate transfer function from mtm 
    ...
    top_mtm(:)   = cmplx(0.,0.)
    bot_mtm(:)   = cmplx(0.,0.)
    trans_func(:) = cmplx(0.,0.)
 
    do ictaper = 1, ntaper

      dat2_dtw_ho(:) = cmplx(0.,0.) ! note: this has to be initialized inside the loop
      dat1_dtw_ho(:) = cmplx(0.,0.)

      ! apply time-domain taper
      do i = 1, nlen
        dat2_dtw_h(i) = dat2(i) * tas(i,ictaper)     ! single-tapered, windowed data2
        dat1_dtw_h(i) = dat1(i) * tas(i,ictaper)  ! single-tapered, windowed, shifted data1
      enddo

      dat2_dtw_ho(1:nlen) = cmplx(dat2_dtw_h(1:nlen),0.)
      dat1_dtw_ho(1:nlen) = cmplx(dat1_dtw_h(1:nlen),0.)

      ! apply FFT to get complex spectra
      call fft(LNPT,dat2_dtw_ho,FORWARD_FFT,deltat)
      call fft(LNPT,dat1_dtw_ho,FORWARD_FFT,deltat)
   ...
   ! water level for transfer function
    ...
    do i =  i_fstart,i_fend
      if(abs(bot_mtm(i)) > abs(wtr_mtm_use)) trans_func(i) = top_mtm(i) / bot_mtm(i)
      if(abs(bot_mtm(i)) < abs(wtr_mtm_use)) trans_func(i) = top_mtm(i) / (bot_mtm(i)+wtr_mtm_use)
    enddo
   ...
   ! print*,'water level for multi taper transfer function is : ',wtr_use
    ...
    dat1_dtwo = cmplx(0.,0.)
    dat2_dtwo = cmplx(0.,0.)
    dat1_dtwo(1:nlen) = cmplx(dat1(1:nlen),0.0)
    dat2_dtwo(1:nlen) = cmplx(dat2(1:nlen),0.0)
    call fft(LNPT,dat1_dtwo,FORWARD_FFT,deltat)
    call fft(LNPT,dat2_dtwo,FORWARD_FFT,deltat)
    !open(1,file='OUTPUTS/top_bot_mtm',status='unknown')
    open(3,file=trim(output_dir)//'/trans_func',status='unknown')
    open(4,file=trim(output_dir)//'/trans_error',status='unknown')
    do  i =  i_fstart,i_fend
   ! write(1,*) wvec(i)/TWOPI, abs(top_mtm(i)),abs(bot_mtm(i))
    write(3,'(2E15.5)') wvec(i)/TWOPI, abs(trans_func(i))
    write(4,'(4E15.5)') wvec(i)/TWOPI, abs(dat1_dtwo(i)),abs(dat2_dtwo(i)),abs(trans_func(i)*dat2_dtwo(i))
    enddo
   ! close(1)
    close(3)
    close(4)
    ...
    if (ntaper > 1 .and. USE_ERROR_MT) then
      ...
      do iom = 1, ntaper
        ...
        do ictaper = 1, ntaper
          ...
          dat2_dtw_h(1:nlen) = dat2(1:nlen) * tas(1:nlen,ictaper)
          dat1_dtw_h(1:nlen) = dat1(1:nlen) * tas(1:nlen,ictaper)
...
! ------------------------------------------------------------------------------------
  ...
  implicit none
    ...
    double precision, dimension(*), intent(in) :: syn
    double precision, intent(in) ::  deltat,df,err_dt_cc,err_dlnA_cc
    integer, intent(in) :: i_fstart,i_fend
    double precision, dimension(*), intent(in) :: dtau_w,dlnA_w,err_dtau_mt,err_dlnA_mt
    double precision, dimension(*), intent(out) :: fp, fq 

    integer :: i,ictaper
    ! frequency-domain taper 
    integer :: nfrange, window_type =3
    double precision, dimension(NPT) :: w_taper, wp_taper, wq_taper, ey1, ey2
    double precision :: ffac, dtau_wtr, dlnA_wtr, err_t, err_A
    
    ! mtm adj
    double precision, dimension(NPT) :: syn_dtw_h, syn_vtw_h
    complex*16, dimension(:,:),allocatable ::  syn_dtw_ho_all,syn_vtw_ho_all
    complex*16, dimension(NPT) :: p_bot_mtm, q_bot_mtm
    complex*16, dimension(NPT) :: pwc_adj,qwc_adj
    double precision, dimension(NPT) :: dtau_pj_t, dlnA_qj_t
    double precision :: ampmax,wtr_use
...
!          ! CC error estimate
    ...
    ! calculate the tapers
     ...
     call staper(nlen, NW, ntaper, tas, NPT, ey1, ey2)
      ...
      do ictaper = 1,ntaper
        ...
        syn_dtw_h(1:nlen) = syn(1:nlen) * tas(1:nlen,ictaper)
        ...
        call fft(LNPT,syn_dtw_ho_all(:,ictaper),FORWARD_FFT,deltat)
        call fft(LNPT,syn_vtw_ho_all(:,ictaper),FORWARD_FFT,deltat)
   ...
   if( DISPLAY_DETAILS) then
   ! open(2,file='OUTPUTS/adj_bot_pq',status='unknown')
   ! do  i = i_fstart,i_fend 
        ...
        !             uncertainty estimations
         ! adds misfit measurement dtau, dlnA
          pwc_adj(i_fstart: i_fend) = pwc_adj(i_fstart: i_fend) &
                        * cmplx(dtau_w(i_fstart: i_fend),0.) &
                        * cmplx(wp_taper(i_fstart: i_fend),0.)
          qwc_adj(i_fstart: i_fend) = qwc_adj(i_fstart: i_fend) &
                        * cmplx(dlnA_w(i_fstart: i_fend),0.) &
                        * cmplx(wq_taper(i_fstart: i_fend),0.)

        ! IFFT into the time domain
        call fftinv(LNPT,pwc_adj,REVERSE_FFT,deltat,dtau_pj_t)
        call fftinv(LNPT,qwc_adj,REVERSE_FFT,deltat,dlnA_qj_t)

        ! create adjoint source
        ! applies taper to time signal
        fp(1:nlen) = fp(1:nlen) + tas(1:nlen,ictaper) * dtau_pj_t(1:nlen)
        fq(1:nlen) = fq(1:nlen) + tas(1:nlen,ictaper) * dlnA_qj_t(1:nlen)
...
! ------------------------------------------------------------------------------------
  ...
  implicit none
    ...
    integer, intent(in) :: npts,nlen
    double precision, dimension(*), intent(in) :: s1,s2
    double precision, intent(in) :: deltat,df
    double precision, intent(in) :: err_dt_cc_obs,err_dt_cc_syn,err_dlnA_cc_obs,err_dlnA_cc_syn
    integer, intent(in) :: i_fstart,i_fend
    double precision, dimension(*), intent(in) :: ddtau_w,ddlnA_w
    double precision, dimension(*), intent(in) :: err_dtau_mt_obs,err_dtau_mt_syn,err_dlnA_mt_obs,err_dlnA_mt_syn
    double precision, dimension(*), intent(out) :: fp1,fp2,fq1,fq2

    integer :: i,ictaper
    ! frequency-domain taper 
    integer :: nfrange, window_type =3
    double precision, dimension(NPT) :: w_taper, wp_taper, wq_taper, ey1, ey2
    double precision :: ffac, ddtau_wtr, ddlnA_wtr, err_t, err_A

    ! mtm adj
    double precision, dimension(NPT) :: s1_dtw_h, s1_vtw_h,s2_dtw_h, s2_vtw_h
    complex*16, dimension(:,:),allocatable ::  s1_dtw_ho_all,s1_vtw_ho_all
    complex*16, dimension(:,:),allocatable ::  s2_dtw_ho_all,s2_vtw_ho_all
    complex*16, dimension(NPT) :: Mtr1_mtm,Mtr2_mtm,Mtr3_mtm,Mtr4_mtm,Mtr5_mtm
    complex*16, dimension(NPT) :: p1_adj,pw1_adj,p2_adj,pw2_adj
    complex*16, dimension(NPT) :: q1_adj,qw1_adj,q2_adj,qw2_adj
    complex*16, dimension(NPT) :: fp1_adj,fp2_adj,fq1_adj,fq2_adj
    double precision, dimension(NPT) :: fp1_adj_t,fp2_adj_t,fq1_adj_t,fq2_adj_t
    double precision, dimension(:,:),allocatable :: tas
...
!          ! CC error estimate
   ...
   !! constant terms for adj 
      ...
      do ictaper = 1,ntaper
        ...
        ! tapered synthetic displacement
        s1_dtw_h(1:nlen) = s1(1:nlen) * tas(1:nlen,ictaper)
        s2_dtw_h(1:nlen) = s2(1:nlen) * tas(1:nlen,ictaper)
        ...
        s1_dtw_ho_all(1:nlen,ictaper) = cmplx(s1_dtw_h(1:nlen),0.)
        s1_vtw_ho_all(1:nlen,ictaper) = cmplx(s1_vtw_h(1:nlen),0.)
        s2_dtw_ho_all(1:nlen,ictaper) = cmplx(s2_dtw_h(1:nlen),0.)
        s2_vtw_ho_all(1:nlen,ictaper) = cmplx(s2_vtw_h(1:nlen),0.)

        ! apply FFT get complex spectra
        call fft(LNPT,s1_dtw_ho_all(:,ictaper),FORWARD_FFT,deltat)
        call fft(LNPT,s1_vtw_ho_all(:,ictaper),FORWARD_FFT,deltat)
        call fft(LNPT,s2_dtw_ho_all(:,ictaper),FORWARD_FFT,deltat)
        call fft(LNPT,s2_vtw_ho_all(:,ictaper),FORWARD_FFT,deltat)
      ...
      do ictaper = 1,ntaper
       ...
       !                 - s2_dtw_ho_all(i,ictaper) / Mtr5_mtm(i)
        ...
        !             uncertainty estimations
         ! adds misfit measurement dtau, dlnA
          fp1_adj(i_fstart: i_fend) &
                        ...
                        * cmplx(ddtau_w(i_fstart: i_fend),0.) &
                        * cmplx(wp_taper(i_fstart: i_fend),0.)
          ...
          fq1_adj(i_fstart: i_fend) &
                        ...
                        * cmplx(ddlnA_w(i_fstart: i_fend),0.) &
                        * cmplx(wq_taper(i_fstart: i_fend),0.)
        ...
        ! IFFT into the time domain
        call fftinv(LNPT,fp1_adj,REVERSE_FFT,deltat,fp1_adj_t)
        call fftinv(LNPT,fp2_adj,REVERSE_FFT,deltat,fp2_adj_t)
        call fftinv(LNPT,fq1_adj,REVERSE_FFT,deltat,fq1_adj_t)
        call fftinv(LNPT,fq2_adj,REVERSE_FFT,deltat,fq2_adj_t)

        ! create adjoint source
        ! applies taper to time signal
        fp1(1:nlen) = fp1(1:nlen) + tas(1:nlen,ictaper) * fp1_adj_t(1:nlen)
        fp2(1:nlen) = fp2(1:nlen) + tas(1:nlen,ictaper) * fp2_adj_t(1:nlen)
        fq1(1:nlen) = fq1(1:nlen) + tas(1:nlen,ictaper) * fq1_adj_t(1:nlen)
        fq2(1:nlen) = fq2(1:nlen) + tas(1:nlen,ictaper) * fq2_adj_t(1:nlen)
      enddo  ! ictaper
      deallocate(tas)
...
implicit none
  ...
  ! inputs & outputs 
  double precision, dimension(*), intent(in) :: d1,d2
  ...
  double precision, dimension(npts) :: d1_tw,d2_tw
  ! cc 
  integer :: ishift
  double precision :: dlnA
...
call xcorr_calc(d1_tw,d2_tw,npts,1,nlen,ishift,dlnA,cc_max) !T(d1-d2)
  ...
  implicit none
    ...
    integer :: i,fnum 
    double precision :: df,ampmax, wtr_use
    integer :: i_ampmax, i_fstart_stop, i_fend_stop
    complex*16, dimension(NPT) :: dat_dtwo     
...
!! find reliable frequency limits 
    ...
    df = 1./(NPT*deltat)
    dat_dtwo = cmplx(0.,0.)
    dat_dtwo(1:nlen) = cmplx(dat(1:nlen),0.0)
    call fft(LNPT,dat_dtwo,FORWARD_FFT,deltat)
...
! water level based untapered dat1
 ...
 ! i_fend 
  ...
  ! NOTE: We implement the exact equations that are in the Latex notes.
    ...
    use user_parameters
    implicit none
    double precision, dimension(*), intent(in) :: data_dtw, syn_dtw
    integer, intent(in) :: npts,nlen, ishift
    double precision, intent(in) :: deltat, dlnA
    double precision, intent(inout) :: sigma_dt, sigma_dlnA

    double precision, dimension(npts) :: syn_dtw_cc, syn_vtw_cc
  ...
  subroutine write_phase(phase_func, wvec, i_right, tshift, phi_wt, dtau_wt)
   ...
   implicit none
    ...
    complex*16, dimension(*), intent(in) :: phase_func
    double precision, dimension(*), intent(in) :: wvec
    double precision, intent(in) ::  tshift
    integer, intent(in) ::  i_right
    double precision, dimension(*), intent(out) :: phi_wt, dtau_wt
 ...
 ! ---------------------------------------------------------------------------
  ...
  subroutine write_trans(trans, wvec,i_left, i_right, tshift, dlnA,phi_wt, abs_wt, dtau_wt, dlnA_wt)
    ...
    use user_parameters
    implicit none
    complex*16, dimension(*),intent(in) :: trans
    double precision, dimension(*),intent(in) :: wvec
    double precision, intent(in) :: tshift, dlnA
  ...
  implicit none
    ...
    double precision, dimension(*),intent(out) :: phi_wt
    integer, intent(in) :: i_left, i_right
  ...
  implicit none
    ...
    integer :: i
    double precision :: dp, dps, dp_corr
...
! input parameters
  ...
  double precision, dimension(*), intent(out) :: dat,syn
 ...
 double precision :: fac
  ...
  implicit none
    ...
    double precision, dimension(*), intent(in) :: dat
    double precision, intent(in) :: dlnA
    integer, intent(in) :: npts, istart,iend, ishift,window_type
    integer, intent(out) :: nlen
    double precision, dimension(*), intent(out) :: dat_win

    integer ::  i, j
    double precision :: sfac1, ipwr_t, fac
  ...
  subroutine cc_window_inverse(dat_win,npts,window_type,istart,iend,ishift,dlnA,dat)
   ...
   implicit none
    ...
    double precision, dimension(*), intent(in) :: dat_win
    double precision, intent(in) :: dlnA
    integer, intent(in) :: npts, istart,iend, ishift,window_type
    double precision, dimension(*), intent(out) :: dat
 ...
 !         endif
  ...
  double precision, intent(in) :: deltat 
  integer, intent(in) :: npts, nlen
  double precision, dimension(*),intent(out) :: syn_vel
